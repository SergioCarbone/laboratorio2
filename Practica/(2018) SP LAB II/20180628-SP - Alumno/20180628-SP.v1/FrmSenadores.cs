...
namespace _20180628_SP.v1
{
    public partial class FrmSenadores : Form
    {
        ...
        public void ManejadorVoto(string senador, Votacion.EVoto voto)
        {
            ...
            else
            {
                ...
                if (aux == 0)
                {
                    ...
                    // Guardar resultados
                    dao = new Dao();
                    serializar = new SerializarXML();
                    try
                    {
                        dao.Guardar("ruta", votacion);
                    }
                    catch(Exception e)
                    {
                    ...
                    try
                    {
                        string ruta = Environment.GetFolderPath(Environment.SpecialFolder.Desktop) + "/" + "votacion.xml";
                        serializar.Guardar(ruta, votacion);
                    }
                    catch (Exception e)
                    {
                        MessageBox.Show(e.Message);
        ...
        private void btnSimular_Click(object sender, EventArgs e)
        {
            ...
            // THREAD
            hilo = new System.Threading.Thread(votacion.Simular);
            hilo.Start();
